...
class IrUiMenu(models.Model):
    ...
    def get_user_roots(self):
        ...
        """
        user_domain = []
        if self.env.user.menu_ids:
            user_domain = [('id', 'in', self.env.user.menu_ids.ids)]
        return self.search([('parent_id', '=', False)] + user_domain)
    ...
    def load_menus(self, debug):
        ...
        if self.env.user.menu_ids:
            user_domain = [('id', 'in', self.env.user.menu_ids.ids)]
        menus = self.search([('id', 'child_of', menu_roots.ids)] + user_domain)
