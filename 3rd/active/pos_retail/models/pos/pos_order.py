...
class pos_order(models.Model):
    ...
    def create(self, vals):
        ...
        combo_items = None
        if vals.get('combo_items', None):
            combo_items = vals.get('combo_items')
            del vals['combo_items']
        order = super(pos_order, self).create(vals)
        if combo_items:
            order.create_picking_combo(combo_items)
    ...
    def _order_fields(self, ui_order):
        ...
        if ui_order.get('combo_items', False):
            order_fields.update({
                'combo_items': ui_order['combo_items']
            })
        return order_fields
    ...
    def create_from_ui(self, orders):
        ...
        for o in orders:
            ...
            for line_val in lines:
                line = line_val[2]
                new_line = {}
                if line.get('combo_item_ids'):
                    for product_id, qty in line.get('combo_item_ids').items():
                        line.get('combo_item_ids')[product_id] = qty * line['qty']
                        if not data['combo_items'].get(product_id, None):
                            data['combo_items'][product_id] = line.get('combo_item_ids')[product_id]
                        else:
                            data['combo_items'][product_id] += line.get('combo_item_ids')[product_id]
    ...
    def create_picking_combo(self, combo_items):
        if combo_items:
            ...
            for product_id, quantity in combo_items.items():
