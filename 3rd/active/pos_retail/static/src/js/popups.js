odoo.define('pos_retail.popups', function (require) {
    ...
    var popup_selection_combos = PopupWidget.extend({ // select combo
        ...
        show: function (options) {
            ...
            this.selected_orderline = selected_orderline
            if (!selected_orderline.combo_item_ids) {
                selected_orderline.combo_item_ids = []
            }
            this.combo_item_ids = selected_orderline.combo_item_ids || []
            this._super(options);
            for (let product_id in this.combo_item_ids) {
                const $qty = self.$('article[data-product-id="' + product_id + '"] .cart_qty');
            ...
            this.$('.product').click(function () {
                ...
                const comboItemId = parseInt($(this).data('comboitemId'));
                const comboItem = self.pos.combo_item_by_id[comboItemId]
                const selectedProductId = parseInt($(this).data('productId'));
                let totalItemsSelectedOfCombo = 0
                for (let product_id in self.combo_item_ids) {
                    if (comboItem['product_ids'].indexOf(parseInt(product_id)) != -1) {
                        totalItemsSelectedOfCombo += self.combo_item_ids[product_id]
                    }
                }
                if (totalItemsSelectedOfCombo >= comboItem['quantity']) {
                    return self.wrong_input('span[class="card-issue"]', comboItem['name'] + ' Maximum items can add is: ' + comboItem['quantity']);
                }
                if (self.combo_item_ids[selectedProductId]) {
                    self.combo_item_ids[selectedProductId] += 1
                } else {
                    self.combo_item_ids[selectedProductId] = 1
                }
                const $qty = self.$('article[data-product-id="' + selectedProductId + '"] .cart_qty');
                $qty.html(self.combo_item_ids[selectedProductId])
                $qty.removeClass('oe_hidden')
                selected_orderline.combo_item_ids = self.combo_item_ids
                selected_orderline.trigger('change', selected_orderline);
                selected_orderline.trigger('trigger_update_line');
            });
            this.$('.product .remove-item').click(function () {
                const selectedProductId = parseInt($(this).parent().data('productId'));
                self.remove_item = true
                if (self.combo_item_ids[selectedProductId]) {
                    if (self.combo_item_ids[selectedProductId] > 1) {
                        self.combo_item_ids[selectedProductId] -= 1
                    } else {
                        delete self.combo_item_ids[selectedProductId]
                    }
                }
                const $qty = self.$('article[data-product-id="' + selectedProductId + '"] .cart_qty');
                if (self.combo_item_ids[selectedProductId]) {
                ...
                if (self.combo_item_ids[selectedProductId] <= 0) {
                    delete self.combo_item_ids[selectedProductId]
                }
                selected_orderline.combo_item_ids = self.combo_item_ids
                selected_orderline.trigger('change', selected_orderline);
        ...
        click_confirm: function () {
            ...
            if (picking_vals['move_lines'].length > 0) {
                return rpc.query({
                    ...
                    order['internal_ref'] = picking.name
