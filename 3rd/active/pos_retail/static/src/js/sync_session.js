odoo.define('pos_retail.synchronization', function (require) {
    ...
    exports.pos_bus = Backbone.Model.extend({
        ...
        set_offline: function (total_error) {
            ...
            setTimeout(function () {
                self.pos.set('synch', {state: 'disconnected', pending: total_error});
            }, 2000)
        ...
        register_point: function () {
            // 1. send_notification (selected Order)
            // 2. register_point
            // 3. sync_lines || repush_to_another_sessions
            // 4. call again send_notification
            this.sync_lines();
            this.repush_to_another_sessions();
            var self = this;
            var current_screen = this.pos.gui.get_current_screen();
            if (['floors', 'products', 'kitchen_screen'].indexOf(current_screen) == -1) {
                this.pos.set('synch', {state: 'connecting', pending: 'Pending'});
                return true;
            }
            this.pos.set('synch', {state: 'connecting', pending: 'Synchronize'});
            for (var i = 0; i < this.pos.iot_connections.length; i++) { //
                var iot_connection = this.pos.iot_connections[i];
                var params = {
                    database: this.pos.session.db,
            ...
            for (var i = 0; i < this.pos.iot_connections.length; i++) { //
                ...
                if (this.pos.sync_status) {
                    ...
                    sending().then(function () {
                        self.set_online();
                        console.log('{sync_session.js} save/sync succeed')
                    }, function (err) {
                        self.set_offline();
