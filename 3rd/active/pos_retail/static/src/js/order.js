...
odoo.define('pos_retail.order', function (require) {
    ...
    models.Order = models.Order.extend({
        ...
        set_pricelist: function (pricelist) {
            ...
            if (this.pos.gui.screen_instances['products']) {
                this.pos.auto_update_stock_products(this.pos.db.get_products());
            }
            return res;
        },
        ...
        set_client: function (client) {
            ...
            if (client && client['discount_first_order'] > 0) {
                for (let i = 0; i < this.orderlines.models.length; i++) {
                    let line = this.orderlines.models[i]
                    line.set_discount(client['discount_first_order'])
                }
            }
            return res
        },
        ...
        validate_payment: function () {
            const linePriceSmallerThanZero = this.orderlines.models.find(l => l.get_price_with_tax() <= 0 && !l.promotion)
            if ((!this.is_validate && this.pos.config.validate_payment) || (this.pos.config.apply_validate_return_mode && linePriceSmallerThanZero && !this.is_validate)) { // TODO: validate payment
                this.pos.gui.show_screen('products');
                this.pos._validate_by_manager("this.pos.get_order().is_validate = true; this.pos.gui.show_screen('payment');");
            } else {
        ...
        add_product: function (product, options) {
            var self = this;
            var qty_onhand_of_product = this.pos.db.stock_datas[product.id];
            if (!this.pos.config.allow_order_out_of_stock && product.type == 'product' && qty_onhand_of_product != undefined && qty_onhand_of_product <= 0) {
                return this.pos.gui.show_popup('dialog', {
                    title: 'Warning',
    ...
    models.Orderline = models.Orderline.extend({
        ...
        init_from_JSON: function (json) {
            ...
            if (json.combo_item_ids) {
                this.combo_item_ids = json.combo_item_ids
            }
        ...
        export_as_JSON: function () {
            ...
            if (this.combo_item_ids) {
                json.combo_item_ids = this.combo_item_ids
            }
            if (this.uom_id) {
        ...
        change_combo: function () {
            ...
            if (combo_items.length) {
                this.pos.gui.show_popup('popup_selection_combos', {
                    title: 'Please choose Items Of Combo of ' + this.product.name,
        ...
        set_quantity: function (quantity, keep_price) {
            ...
            if (this.pos.the_first_load == false && quantity != 'remove' && this.pos.config['allow_order_out_of_stock'] == false && quantity && quantity != 'remove' && this.order.syncing != true && this.product['type'] != 'service') {
                ...
                if (this.pos.db.stock_datas[this.product.id] && current_qty > this.pos.db.stock_datas[this.product.id] && this.product['type'] == 'product') {
                    ...
                    return this.pos.gui.show_popup('dialog', {
                        title: 'Warning',
                        body: product['name'] + ' available for sell is ' + this.pos.db.stock_datas[this.product.id],
                    });
                }
            }
