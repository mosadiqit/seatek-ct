odoo.define('pos_retail.restaurant', function (require) {
        ...
        get_line_diff_hash: function () {
            ...
            if (this.combo_item_ids) {
                for (let product_id in this.combo_item_ids) {
                    str += '|' + product_id + '/' + this.combo_item_ids[product_id];
                }
            }
            return str
        },
    ...
    _super_order.computeChanges = function (categories) {
        ...
        for (line_hash in current_res) {
            ...
            if (typeof old === 'undefined') {
                add.push({
                    ...
                    'tags': curr.tags,
                    'combo_item_ids': curr.combo_item_ids,
                    'state': curr.state,
        ...
        for (line_hash in old_res) {
            if (typeof current_res[line_hash] === 'undefined') {
                ...
                rem.push({
                    ...
                    'tags': old.tags,
                    'combo_item_ids': old.combo_item_ids,
                    'state': 'Cancelled',
                    'category': pos_categ_id,
                    'time': hours + ':' + minutes,
                });
        ...
        if (add && add.length > 0) {
            console.log('New requests to Kitchen Printer')
        }
        return {
    ...
    _super_order.build_line_resume = function () {
        ...
        this.orderlines.each(function (line) {
            ...
            if (typeof resume[line_hash] === 'undefined') {
                ...
                if (line.combo_item_ids) {
                    resume[line_hash]['combo_item_ids'] = line['combo_item_ids']
                }
            } else {
