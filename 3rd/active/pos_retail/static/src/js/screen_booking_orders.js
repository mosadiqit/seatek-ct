                    ...
                    for (var i = 0; i < lines.length; i++) {
                        ...
                        if (!product) {
                            ...
                            if (product.type == 'product') {
                                var qty_onhand_of_product = self.pos.db.stock_datas[product.id];
                                if (!self.pos.config.allow_order_out_of_stock && qty_onhand_of_product != undefined && (qty_onhand_of_product <= 0 || qty_onhand_of_product < line.product_uom_qty)) {
                                    order.temporary = true;
                                    order.destroy({'reason': 'abandon'});
                                    return self.pos.gui.show_popup('dialog', {
                                        title: 'Warning, Order Line have one Product out of Stock',
                                        body: product.display_name + ' Have stock on hand is : ' + qty_onhand_of_product + '. Not available for sale',
                                    });
                                }
                            }
                            added_line = true;
