...
odoo.define('pos_retail.screen_order_widget', function (require) {
    ...
    screens.OrderWidget.include({
        ...
        _keyboardHandler: function () {
            ...
            if (current_screen != 'products' || this.gui.has_popup()) {
                this.buffered_key_events = [];
                return true;
            }
            for (let i = 0; i < this.buffered_key_events.length; i++) {
                ...
                if (event.keyCode === 32) { // Space
                    ...
                    key = '+';
                } else if (event.keyCode === 65 && this.pos.config.add_client) {  // key: a
                    $('.add-new-customer').click();
                } else if (event.keyCode === 66) {  // key: b
                    $('.show_hide_buttons').click()
                } else if (event.keyCode === 67 && this.pos.config.allow_customer) { // key: c
                    $('.set-customer').click()
                } else if (event.keyCode === 68 && this.pos.config.allow_discount) { // key: d
                    this.numpad_state.changeMode('discount');
                    this.pos.trigger('change:mode');
                } else if (event.keyCode === 87) { // key: w
                    this.pos.gui.show_popup('popup_create_product', {
                        title: 'Add New Product',
                    })
                } else if (event.keyCode === 72) { // key: h
                    $('.show_hide_pad').click();
                } else if (event.keyCode === 70 && this.pos.config.quickly_payment_full && this.pos.config.quickly_payment_full_journal_id) { // key: f
                    $('.quickly_paid').click();
                } else if (event.keyCode === 103) {

                } else if (event.keyCode === 73 && this.pos.config.management_invoice) { // key: i
                    this.pos.gui.show_screen('invoices');
                } else if (event.keyCode === 75) { // key: k
                ...
                } else if (event.keyCode === 78 && this.pos.config.allow_add_order) { // key: n
                    this.pos.add_new_order();
                } else if (event.keyCode === 79 && this.pos.config.pos_orders_management) { // key: o
                    this.pos.gui.show_screen('pos_orders_screen');
                } else if (event.keyCode === 80 && this.pos.config.allow_price) { // key: p
                    this.numpad_state.changeMode('price');
                    this.pos.trigger('change:mode');
                } else if (event.keyCode === 81 && this.pos.config.allow_qty) { // key q
                    this.numpad_state.changeMode('quantity');
                    this.pos.trigger('change:mode');
                } else if (event.keyCode === 82 && this.pos.config.allow_remove_order) { // key: r
                    $('.deleteorder-button').click();
                } else if (event.keyCode === 83) { // key:  s
                    $('.search-products').focus();
                } else if (event.keyCode === 86) { // key:  v
                    this.gui.chrome.widget['products_view_widget'].el.click();
                } else if (event.keyCode === 87) { // key:  w
