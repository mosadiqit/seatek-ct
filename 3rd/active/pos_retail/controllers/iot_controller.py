...
class SyncController(web.Home):
    ...
    def register_sync(self, database, config_id, session_id, config_ids, sync_multi_session_offline):
        if sync_multi_session_offline:
            ...
            return driver.get_notifications(database, config_id)
        else:
            return self.get_sync(database, config_id, session_id, sync_multi_session_offline)
    ...
    def get_sync(self, database, config_id, session_id, sync_multi_session_offline):
        _logger.info('{iot_controller.py} get_sync of POS Config : %s' % config_id)
        if sync_multi_session_offline:
            ...
            if len(log_ids) > 0:
                log_ids.append(0)
                request.env.cr.execute("delete from pos_sync_session_log WHERE id in %s",  (tuple(log_ids),))
                request.env.cr.commit()
                _logger.info('{iot_controller.py} deleted logs: %s' % log_ids)
