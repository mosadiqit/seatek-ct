...
class consignment_process_ept(models.Model):
    ...
    consignment_process_line_ids = fields.One2many('consignment.process.line.ept','consignment_process_id',string = 'Product Lines', copy=True)
    ...
    def onchange_consignee_source_id(self):
        ...
        if self.consignment_type == 'internal' and self.consignee_source_id:
            return {'domain': {'consignee_dest_id' : [('id','!=',self.consignee_source_id.id),('is_consignee_customer','=',True)]}}
    ...
    def prepare_stock_picking_vals(self):
        ...
        '''
        new_picking_record = self.env['stock.picking'].new({'picking_type_id': self.warehouse_id.int_type_id.id or self.consignee_source_id.consignment_location_id.get_warehouse().int_type_id.id, 'origin' : self.name})
        ...
        if result:
                ...
                if picking_id:
                    ...
                    for line in self.consignment_process_line_ids:
                        vals = {'product_id':line.product_id.id,'picking_type_id': picking_id.picking_type_id.id or False,'product_uom':line.product_id.uom_id.id,'product_qty':line.quantity,'date':str(self.date)}
