...
class res_partner_ept(models.Model):
    ...
    def _create_consignee_location(self,location_name):
        ...
        view_location_id = self.env['stock.warehouse'].search([('is_consignment_warehouse','=',1)]).view_location_id.id
        location_id = location_obj.with_context(active_test=False).search([('location_id','=',view_location_id),('name','=',location_name)])
        if location_id:
            return location_id
    ...
    def _prepare_procurement_vals(self,consignee_location_id,consignee_location_name):
        ''' It will prepare vals for create procurement rule
            ...
            @return: Prepare Vals
        '''
        warehouse_id = self.env['stock.warehouse'].search([('view_location_id','=',consignee_location_id.location_id.id)])
    ...
    def _create_consignee_route(self,consignee_location_id,consignee_location_name):
        ...
        route_id = location_route_obj.with_context(active_test=False).search([('name','=',consignee_location_name + ' || Ship Only')])
    ...
    def _check_consignee_location_name(self):
        ''' This constrain check that Consignment Location name is unique
            ...
            @return : True / False 
        '''
        name = self.search([('consignment_location_name','=ilike',self.consignment_location_name),('id','!=',self.id)])
    ...
    def write(self, vals):
        ...
        if 'consignment_location_name' in vals: 
            ...
            if not locations_to_update or not routes_to_update or not rule_ids_update:
        ...
        if 'is_consignee_customer' in vals:
            if vals['is_consignee_customer'] == False: 
                ...
                self.consignment_route_id.rule_ids.write({'active' : False})
                ...
                if self.consignment_location_id and self.consignment_route_id:   
                    ...
                    self.consignment_route_id.with_context(active_test=False).rule_ids.write({'active':True})
                ...
                if not self.consignment_route_id:
                    ...
                    route_id.with_context(active_test=False).rule_ids.write({'active':True})
        ...
        if result and locations_to_update and routes_to_update:
            ...
            rule_ids_update.write({'name' : 'Consignee: ' + new_name + ' -> Customer'})
