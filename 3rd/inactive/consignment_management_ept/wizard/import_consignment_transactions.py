...
class import_consignment_transactions(models.TransientModel):
    ...
    def download_template(self):
        ...
        try:
            if (self.transaction_type == 'transfer' or self.transaction_type == 'return'):
                attachment=self.env['ir.attachment'].search([('id','=',self.env.ref('consignment_management_ept.ir_attachment_import_transfer_return_template_ept').id)])           
            elif(self.transaction_type == 'order'):
                attachment=self.env['ir.attachment'].search([('id','=',self.env.ref('consignment_management_ept.ir_attachment_import_order_template_ept').id)]) 
            elif(self.transaction_type == 'internal'):
                attachment=self.env['ir.attachment'].search([('id','=',self.env.ref('consignment_management_ept.ir_attachment_import_internal_template_ept').id)]) 
            ...
            _logger.error(e)
            raise Warning ("Hello %s, Something Went Wrong, Please try again or you should contact your Administrator"%(self.env.user.name)) 
    ...
    def do_import(self):
        ...
        return {'effect': {'fadeout': 'slow', 'message': "Yeah %s, It's Done, You can check consignment logs for further details."%self.env.user.name,
        ...
        try :        
            for state, list in dynamic.items():
                if list:
                    ...
                    main_file = self.env['ir.attachment'].create(vals)
                    log_id.write({
                            'main_attachment' : main_file.id
                        }) 
                    ...
                    log_line_data= self.env['consignment.log.line.ept'].create(vals_list)
                    ...
                    if state == 'pending' and list:
                        ...
                        pending_file = self.env['ir.attachment'].create(vals)                                           
                        log_id.write({
                                'pending_attachment' : pending_file.id
                            })
            return done_log_id
    ...
    def validate_for_order(self, data): 
        ...
        try:
            for row in data:
                ...
                """         
                if msg:
                    pending_log_lines_list.append({'msg' : msg, 'line_no' : line_no, 'consignee' : source_partner ,'product' : product_name, 'qty' : qty})
                                                          
                else:
                    done_log_lines_list.append({'msg':'Successfully Done' ,'line_no' : line_no, 'consignee' : source_partner,'product' : product_name, 'qty' : qty})                                                                                
