...
#         print(qry)
    ...
    def generate_excel_report(self):
        ...
        header = xlwt.easyxf("font: bold on, height 200; pattern:pattern solid, fore_colour gray25; alignment: horiz center ,vert center ")
        left_align = xlwt.easyxf("font: height 200; alignment: horiz left, vert center; pattern: pattern solid, fore_colour gray25")
        right_align = xlwt.easyxf("font: height 200; alignment: horiz right, vert center; pattern: pattern solid, fore_colour gray25")
        
        xlwt.add_palette_colour("blue", 0x21)
        workbook.set_colour_RGB(0x21, 176, 216, 230)  
        blue_format = xlwt.easyxf("font: height 200,bold on, name Arial; alignment: horiz center, vert center;  pattern: pattern solid, fore_colour blue;  borders: top thin,right thin,bottom thin,left thin")
        
        xlwt.add_palette_colour("pink", 0x17)
        workbook.set_colour_RGB(0x17, 255, 204, 204)  
        pink_format = xlwt.easyxf("font: height 200,bold on, name Arial; alignment: horiz center, vert center;  pattern: pattern solid, fore_colour pink;  borders: top thin,right thin,bottom thin,left thin")
        
        result = self.retrive_data()
        
        if not result:
            return {'effect': {'fadeout': 'slow', 'message': "Hey %s, No Data Found To Generate Report."%self.env.user.name,
                       'img_url': '/web/static/src/img/warning.png', 'type': 'rainbow_man'}}
        ...
        return {
            ...
            'url':'web/content/?model=consignment.stock.report.ept&download=true&field=datas&id=%s&filename= Consignment Report - %s.xls' % (self.id, datetime.now().strftime("%Y-%m-%d")),
            'target': 'new',
             }              
    ...
    def onscreen_report(self):
        ...
        if self.report_type == 'consignee_wise':
            pivot_view = self.env.ref('consignment_management_ept.view_onscreen_consignment_report_consignee_wise_pivot', False)
        elif self.report_type == 'product_wise':
            pivot_view = self.env.ref('consignment_management_ept.view_onscreen_consignment_report_product_wise_pivot', False)
        
        report['views'] = [(tree_view and tree_view.id or False, 'tree'), (pivot_view and pivot_view.id or False, 'pivot')]
                ...
                for record in records:
                    report_records += self.env['consignment.onscreen.report'].create({
                                            ...
                                            'consignee' : self.env['res.partner'].browse(record.get('consignee')).name + '(' +
                                                          self.env['res.partner'].browse(record.get('consignee')).consignment_location_name + ')',
                                            'opening_qty' : record.get('opening_qty'),
                                            'transfer_qty' : record.get('transfer_qty'),
                                            'received_from_another' : record.get('received_from_another'),
                                            'return_qty' : record.get('return_qty'),
